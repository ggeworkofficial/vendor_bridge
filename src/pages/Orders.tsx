...
} from "@/api/complaint-message.api";
import { useOrderStore } from "@/features/order/order.store";
import { useToast } from "@/hooks/use-toast";
import type { OrderStatus } from "@/types/order";

const useComplaintsQuery = (orderId: string | null) =>
  useQuery({
    queryKey: ["complaints", "order", orderId],
    queryFn: async () =>
      await getComplaints({ order_id: orderId || undefined, limit: 100 }),
    enabled: !!orderId,
  });

const useComplaintMessagesQuery = (complaintId: string | undefined) =>
  useQuery({
    queryKey: ["complaint-messages", "complaint", complaintId], // ← unique per complaint
    queryFn: async () =>
      await getComplaintMessages({ complaint_id: complaintId, limit: 100 }),
    enabled: !!complaintId,
  });
...
const Orders = () => {
  ...
  const [selectedOrderIdForComplaints, setSelectedOrderIdForComplaints] =
  ...
  const { data: complaintsResponse, isLoading: complaintsLoading } =
    useComplaintsQuery(selectedOrderIdForComplaints);

  const complaints = complaintsResponse?.data?.data ?? [];

  const { data: messagesResponse, isLoading: messagesLoading } =
    useComplaintMessagesQuery(selectedComplaint?.id);

  const complaintMessages = messagesResponse?.data?.data ?? [];
  ...
  const handleCreateComplaint = async (orderId: string) => {
    ...
    try {
      ...
      toast({
        title: "Create failed",
        description: "Unable to create complaint.",
        variant: "destructive",
      });
    } finally {
  ...
  const handleSendReply = async () => {
    ...
    try {
      await createComplaintMessage({
        complaint_id: selectedComplaint.id,
        message: complaintReply.trim(),
      });
      setComplaintReply("");
      await queryClient.invalidateQueries({ queryKey: ["complaint-messages"] });
    ...
    <Layout>
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      {order.products.map((product) => (
                        ...
                        >
                          <span>
                            Product: {product.id.substring(0, 8)}... ×{" "}
                            {product.quantity}
                          </span>
                          <span>
                            $
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          if (!open) {
            setIsCreateComplaintOpen(false);
            setSelectedOrderIdForComplaints(null);
            setComplaintSubject("");
            setComplaintDescription("");
          }
        }}
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Submit a complaint about your order.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="gap-2">
            ...
            <Button
              onClick={() =>
                selectedOrderIdForComplaints &&
                handleCreateComplaint(selectedOrderIdForComplaints)
              }
      ...
      >
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Order Complaints</DialogTitle>
          </DialogHeader>
            ...
            <div className="space-y-4">
              {selectedComplaint ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-3 max-h-60 overflow-y-auto">
                      {messagesLoading ? (
                        ...
                        complaintMessages.map((msg) => (
                          <div
                            key={msg.id}
                            className={`p-2 rounded-lg ${msg.sender.role === "admin" ? "bg-primary/10 ml-8" : "bg-muted"}`}
                          >
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        disabled={isSendingReply || !complaintReply.trim()}
                      >
                ...
                <div className="flex items-center justify-center h-full">
                  <p className="text-sm text-muted-foreground">
                    Select a complaint to view details and messages.
                  </p>
                </div>
              )}
            </div>
